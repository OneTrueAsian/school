...
void MatrixTester::runTests() {
	cout << "Running Matrix<unsigned> tests..." << endl;
...
void MatrixTester::testEquality() {
	...
	// same sized
	Matrix<unsigned> m3(2, 3);
	Matrix<unsigned> m4a(2, 3);
	Matrix<unsigned> m4b(2, 3);
	Matrix<unsigned> m5(2, 3);
	for (unsigned i = 0; i < m3.getRows(); i++) {
		for (unsigned j = 0; j < m3.getColumns(); j++) {
			m3[i][j] = i+j;
			m4a[i][j] = i+j;
	...
	// different row sizes
	Matrix<unsigned> m6(3,3);
	Matrix<unsigned> m7(2,3);
	assert( !(m6 == m7) );
	cout << " 3 " << flush;

	// same row sizes, different column sizes
	Matrix<unsigned> m8(2,4);
	assert( !(m8 == m7 ) );
	cout << " 4 " << flush;
...
void MatrixTester::testReadSubscript(const Matrix<unsigned>& mat) {
...
void MatrixTester::testSubscripts() {
	...
	try {
		m2[2][0] = 0;
		cerr << "successfully wrote past last Matrix<unsigned> row";
		exit(1);
	} catch (range_error&) {
		cout << " 5 " << flush;
 	}
	...
	try {
		m2[0][3] = 0;
		cerr << "successfully wrote past last Matrix<unsigned> column";
		exit(1);
	} catch (range_error&) {
		cout << " 6 " << flush;
 	}
...
void MatrixTester::testTranspose() {
	...
	// nonempty
	Matrix<unsigned> m1(4, 3);
	for (unsigned i = 0; i < m1.getRows(); i++) {
		for (unsigned j = 0; j < m1.getColumns(); j++) {
			m1[i][j] = i*j;
		}
	}
	Matrix<unsigned> m2 = m1.getTranspose();
...
void MatrixTester::testAddition() {
	cout << "Testing +... " << flush;
	// empty
	Matrix<unsigned> m0a, m0b, m0c(3,2);
	m0c = m0a + m0b;
	...
	// nonempty, same size
	Matrix<unsigned> m1(3, 4);
	Matrix<unsigned> m2(3, 4);
	for (unsigned i = 0; i < m1.getRows(); i++) {
		for (unsigned j = 0; j < m1.getColumns(); j++) {
			m1[i][j] = i*j;
			m2[i][j] = i+j;
...
//	cout << " 0 " << flush;
//
//	Matrix<unsigned> m1(2, 3);
//	Matrix<unsigned> m2(3, 2);
//	for (unsigned i = 0; i < m1.getRows(); i++) {
//		for (unsigned j = 0; j < m1.getColumns(); j++) {
//			m1[i][j] = i+j+1;
//			m2[j][i] = i+j+1;
...
void MatrixTester::testReadFromStream() {
	...
	fin >> rows >> columns;
	Matrix<unsigned> m(rows, columns);
	m.readFrom(fin);
...
void MatrixTester::testWriteToStream() {
	...
	Matrix<unsigned> m;
	// read a Matrix<unsigned> whose values we know
	m.readFrom("MatrixTest1.txt");
...
void MatrixTester::testWriteToFile() {
	...
	Matrix<unsigned> m;
	// read in a Matrix<unsigned> whose values we know
	m.readFrom("MatrixTest1.txt");
	// write it to a file
